...
import api from '../api/axios'; // Nuestra instancia configurada

const Alerts = () => {
  const navigate = useNavigate();
  const { dark } = useTheme();
  
  // Estado para las alertas del backend
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        // Asumiendo que el backend de tus compañeros tiene este endpoint
        const response = await api.get('/alerts');
        setAlerts(response.data);
      } catch (error) {
        console.error("Error fetching alerts:", error);
        // Opcional: Cargar datos locales si falla la API para no ver la pantalla vacía
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          alerts.map((alert, idx) => {
            // Lógica para decidir el color según el nivel de llenado
            ...
            return (
              ...
              >
                ...
                {isCritical && (
                  <button className="mt-3 bg-red-500 text-white text-[10px] px-3 py-1 rounded-lg font-bold hover:bg-red-600">
                    VACIAR AHORA
                  </button>
                )}
              </div>
